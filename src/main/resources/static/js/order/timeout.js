            ...
            if (list.length == 0) {
                ...
                for (var i = 0; i < list.length; i++) {
                    ...
                    if (list[i].orderStatus == 1) {
                        view += '<a class="layui-btn-sm edit' + i + '" onmouseenter="show(\'编辑\', \'.edit' + i + '\')" onmouseleave="hide()" onclick="getbox(\'' + list[i].oid + '\',' + list[i].orderStatus + ',\''+list[i].oCode+'\','+56+')"><i class="layui-icon-set-fill layui-icon"></i></a>';
                    } else {
...
function getbox(id, orderStatus,ocode, actionid) {
    $("#bkTime").hide();
    ...
    layer.open({
        ...
        , btn2: function () {
            ...
            $.ajax({
                type: "POST",
                url: '/orderInfo/updateTimeoutOrder',
                data: data,
                xhrFields: {
                    withCredentials: true
                },
                ...
                success: function (backData) {
                    $.ajax({
                        type: "POST",
                        url: '/log/add',
                        data: {actionid: actionid, remark: "操作：超时结算，订单id："+id+"，订单编号："+ocode},
                        xhrFields: {
                            withCredentials: true
                        },
                        crossDomain: true,
